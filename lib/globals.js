...
const DATE_STRUCTURE_ERROR_TYPES = new Set(['required', 'required-day', 'required-month', 'required-year', 'numeric', 'numeric-day', 'numeric-month', 'numeric-year', 'date-day', 'date-month', 'date-year']);
...
let globals = {
    ...
    hmpoGetErrorSummary(ctx) {
        ...
        for (let error of errors) {
            ...
            if (dateFieldKeys.has(error.key) && error.type === 'required') {
                addError(error);
                continue;
            }
        ...
        for (let fieldName of dateFields) {
            ...
            if (groupError && groupError.type === 'required') continue;
            for (let part of DATE_PARTS) {
