...
const DATE_AGGREGATE_ERROR_TYPES = new Set(['required', 'numeric', 'date']);
const DATE_STRUCTURE_ERROR_TYPES = new Set(['required', 'required-day', 'required-month', 'required-year', 'numeric', 'numeric-day', 'numeric-month', 'numeric-year', 'date', 'date-day', 'date-month', 'date-year']);
...
let globals = {
    ...
    hmpoGetErrorSummary(ctx) {
        ...
        let dateFieldKeys = new Set(dateFields);
        let datePartKeys = new Set(dateFields.flatMap(fieldName =>
            DATE_PARTS.map(part => fieldName + '-' + part)
        ));
        let addError = error => {
            if (!error || errorKeys.has(error.key)) return;
        ...
        for (let error of errors) {
            if (datePartKeys.has(error.key)) continue;

            if (dateFieldKeys.has(error.key) && DATE_AGGREGATE_ERROR_TYPES.has(error.type)) {
                addError(error);
                continue;
            }

            if (!dateFieldKeys.has(error.key)) {
                addError(error);
                continue;
            }
            ...
            for (let part of DATE_PARTS) {
                ...
                let partError = fieldErrors[key] || (error.field === key ? error : undefined);
                addError(partError);
            }

            if (!DATE_STRUCTURE_ERROR_TYPES.has(error.type)) addError(error);
        }

        for (let fieldName of dateFields) {
            let groupError = fieldErrors[fieldName];
            if (groupError && DATE_AGGREGATE_ERROR_TYPES.has(groupError.type)) {
                addError(groupError);
                continue;
            }
            for (let part of DATE_PARTS) {
                ...
                if (groupError && groupError.field === key) continue;
                addError(error);
            }
        }
